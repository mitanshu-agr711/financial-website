...
export const getRecords = async (req, res) => {
    try {
        ...
        const { startDate, endDate, error: dateError } = validateDateRange(req.query.startDate, req.query.endDate);
        if (dateError) {
...
export const updateRecord = async (req, res) => {
    try {
        ...
        if (Object.keys(req.body).length > 0) {
            ...
            const missingInProvided = requiredFields.filter(field => providedFields.includes(field) && (req.body[field] === null || req.body[field] === ''));
...
export const permanentlyDeleteRecord = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
        if (!userId) {
...
export const getBulkRecords = async (req, res) => {
    try {
        const userId = req.userId;
        const { ids } = req.body;
        if (!userId) {
